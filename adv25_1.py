# $ cat 25.txt | python3 adv25_1.py input25.txt
...
class QueueInputSource():
    def __init__(self, inputs, default_val=0):
        self.inputs = []
...
inp = QueueInputSource([])
cmp = IntcodeComputer(mem, inp)
received_chars = 0
out_string = ""
x = 0
y = 0
...
while True:
    cmp.execute_until_output()
    out = "".join([chr(character) for character in cmp.outputs if character < 256])
    cmp.outputs = []
    received_chars = len(out)
    out_string += out
    print(out_string)
    #if "Command?" in out_string:
    match = re.search("Command\?\n$", out_string) #or re.search("\!$", out_string)
    if match:
        print(out_string)
        out_string = ""
        in_string = input("")
        if in_string == "east":
            ...
            x -= 1
        elif in_string == "north":
            y += 1
        elif in_string == "south":
            y -= 1
        in_string += "\n"
        in_list = [ord(character) for character in in_string]
        #inp.add_inputs(in_list)
        inp.inputs = in_list
        #print(inp.inputs)
